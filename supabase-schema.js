/**
 * Converted supabase schema from supabase-schema.sql
 * This module exports the SQL as `sql` and provides an optional runner `run` using 'pg' if you want to execute it.
 *
 * Usage:
 *  - Import the SQL string: const { sql } = require('./supabase-schema.js');
 *  - Or run it directly (requires `pg` and DATABASE_URL env):
 *      node -e "require('./supabase-schema.js').run()"
 *
 * NOTE: Running programmatically requires a Postgres connection string in DATABASE_URL and the `pg` package installed.
 */

const sql = `-- Beth Torah Message Board SaaS - Supabase schema
...
async function run() {
  ...
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL environment variable is required to run the SQL.');
  }
